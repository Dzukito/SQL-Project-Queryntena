...
IF OBJECT_ID('QUERYNTENA.HechosPasajeCompras') IS NOT NULL
	drop table QUERYNTENA.HechosPasajeCompras
GO

IF OBJECT_ID('QUERYNTENA.HechosEstadiaCompras') IS NOT NULL
	drop table QUERYNTENA.HechosEstadiaCompras
GO

IF OBJECT_ID('QUERYNTENA.HechosPasajeVentas') IS NOT NULL
	drop table QUERYNTENA.HechosPasajeVentas
GO

IF OBJECT_ID('QUERYNTENA.HechosEstadiaVentas') IS NOT NULL
	drop table QUERYNTENA.HechosEstadiaVentas
GO

IF OBJECT_ID('QUERYNTENA.Dimension_Ciudad') IS NOT NULL
	drop table QUERYNTENA.Dimension_Ciudad
GO

IF OBJECT_ID('QUERYNTENA.Dimension_Tipo_Pasaje') IS NOT NULL
	drop table QUERYNTENA.Dimension_Tipo_Pasaje
GO


--Se crean las dimensiones restantes y se hacen los inserts
...
insert into QUERYNTENA.Dimension_Tipo_Pasaje(tipo_pasaje_nombre) 
	select distinct buta_tipo from QUERYNTENA.Butaca_Vuelo

insert into QUERYNTENA.Dimension_Ciudad(ciud_nombre) 
	select ruta_ciu_dest from QUERYNTENA.Ruta_Aerea union select ruta_ciu_orig from QUERYNTENA.Ruta_Aerea


--Se crean las tablas de hechos...
...
--Se crean las funciones necesarias para realizar los inserts de las tablas de hechos

IF OBJECT_ID('QUERYNTENA.getCiudadDestinoId') is not null
	DROP FUNCTION QUERYNTENA.getCiudadDestinoId 
GO

--A partir de una ruta aérea retorna la ciudad de destino
...
IF OBJECT_ID('QUERYNTENA.FX_EDAD') IS NOT NULL 
	DROP FUNCTION QUERYNTENA.FX_EDAD
GO

--A partir de la edad del cliente y la fecha de la compra, retorna la edad del cliente en el momento que se realizó la compra
CREATE FUNCTION QUERYNTENA.FX_EDAD (@FECHA_NAC datetime2(3), @FECHA_COMPRA datetime2(3)) 
RETURNS INT
BEGIN
	DECLARE @EDAD INT
...
/*A la hora de insertar los valores, como no se puede tener nulls en las pks decidimos dividir los hechos en 4 tablas diferentes 
...
select year(cest_fecha),month(cest_fecha),cest_empresa,habi_tipo,avg(cest_costo_total) 
from QUERYNTENA.Compra_Estadia
join QUERYNTENA.Habitacion_Estadia on cest_estadia = hest_estadia 
join QUERYNTENA.Habitacion on hest_habitacion_nro = habi_numero and habi_hotel = hest_hotel 
...
select year(fact_fecha),month(fact_fecha),QUERYNTENA.FX_EDAD(clie_fecha_nac,fact_fecha),habi_tipo,avg(vest_precio_final),count(*),sum(vest_precio_final -cest_costo_total) 
from QUERYNTENA.Venta_Estadias join queryntena.Factura on vest_factura = fact_numero 
join QUERYNTENA.Habitacion_Estadia on vest_estadia = hest_estadia 
join QUERYNTENA.Habitacion on hest_habitacion_nro = habi_numero and habi_hotel = hest_hotel
...
update QUERYNTENA.HechosEstadiaCompras 
set hechos_cantidad_camas = (case hechos_tipo_habitacion 
									when 1001 then 1 
									when 1002 then 2
									when 1003 then 3 
									when 1004 then 4
									when 1005 then 5 end)

update QUERYNTENA.HechosEstadiaVentas 
set hechos_cantidad_camas = (case hechos_tipo_habitacion 
									when 1001 then 1 
									when 1002 then 2
									when 1003 then 3 
...
--Tomamos la ciudad destino a la hora de insertar ya que consideramos que es el dato que más valor tiene dentro de la ruta aérea
insert into QUERYNTENA.HechosPasajeCompras(hechos_año,hechos_mes,hechos_proveedor,hechos_tipo_pasaje,hechos_avion,hechos_ruta,hechos_ciudad,hechos_precio_prom_compra_pasajes)
select year(cpas_fecha),month(cpas_fecha),cpas_empresa,tipo_pasaje_id,vuel_avion,vuel_ruta_aerea,QUERYNTENA.getCiudadDestinoId(vuel_ruta_aerea),avg(pasa_costo) 
from QUERYNTENA.Compra_Pasaje
...
--Tomamos la ciudad destino a la hora de insertar ya que consideramos que es el dato que más valor tiene dentro de la ruta aérea
insert into QUERYNTENA.HechosPasajeVentas(hechos_año,hechos_mes,hechos_cliente_edad,hechos_tipo_pasaje,hechos_avion,hechos_ruta,hechos_ciudad,hechos_precio_prom_venta_pasajes,hechos_pasajes_vendidos,hechos_ganancia_pasajes)
select year(fact_fecha),month(fact_fecha),QUERYNTENA.FX_EDAD(clie_fecha_nac,fact_fecha),tipo_pasaje_id,vuel_avion,vuel_ruta_aerea,QUERYNTENA.getCiudadDestinoId(vuel_ruta_aerea),avg(pasa_precio),count(*),sum(pasa_precio-pasa_costo) 
from QUERYNTENA.Venta_Pasaje
